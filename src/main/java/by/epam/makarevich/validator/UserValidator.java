...
public class UserValidator {
    ...
    public boolean validateUserInfo(String name, String surname, String country, String age, String mail,
        ...
        if (!checkWord(name, WORD_REGEX)) {
            isValidInfo = false;
            content.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_NAME, true);
        }
        if (!checkWord(surname, WORD_REGEX)) {
            isValidInfo = false;
            content.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_SURNAME, true);
        }
        if (!checkWord(country, WORD_REGEX)) {
            isValidInfo = false;
            content.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_COUNTRY, true);
        }
        ...
        if (!checkEmail(mail)) {
            isValidInfo = false;
            content.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_EMAIL, true);
        }
        if (!checkWord(login, LOGIN_REGEX)) {
            isValidInfo = false;
            content.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_LOGIN, true);
        }
        if (!checkPassword(password)) {
            isValidInfo = false;
            content.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_PASSWORD, true);
        }
        if (!checkPasswordEquality(password, passwordRepetition)) {
            isValidInfo = false;
            content.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_PASSWORD_REPETITION, true);
        }
        return isValidInfo;
    }
    ...
    private boolean checkAge(String age, RequestContent requestContent) {
        ...
        try {
            ...
            requestContent.getRequestAttributes().put(GeneralConstant.UserPageConstant.WRONG_AGE, true);
            return false;
        }
