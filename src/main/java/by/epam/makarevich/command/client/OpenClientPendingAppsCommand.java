...
public class OpenClientPendingAppsCommand extends AbstractCommand {
    private static final Logger LOGGER = LogManager.getLogger(OpenClientPendingAppsCommand.class);

    public OpenClientPendingAppsCommand(Receiver receiver) {
        super(receiver);
    }
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.OPEN_CLIENT_PENDING_APPS, requestContent);
            ...
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                ...
                LOGGER.log(Level.INFO, "Opening client pending apps command was successful");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with opening client pending apps: " + e.getMessage());
        }
        return router;
    }
}
