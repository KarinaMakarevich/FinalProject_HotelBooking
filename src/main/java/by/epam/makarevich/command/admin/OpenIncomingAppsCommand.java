...
public class OpenIncomingAppsCommand extends AbstractCommand {
    private static final Logger LOGGER = LogManager.getLogger(OpenIncomingAppsCommand.class);

    public OpenIncomingAppsCommand(Receiver receiver) {
        super(receiver);
    }
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.OPEN_INCOMING_APPS, requestContent);
            ...
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                ...
                LOGGER.log(Level.INFO, "Open incoming apps command was successful");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with opening incoming apps " + e.getMessage());
        }
        return router;
    }
}
