...
public class OpenAllAppsCommand extends AbstractCommand {

    private static final Logger LOGGER = LogManager.getLogger(OpenAllAppsCommand.class);

    public OpenAllAppsCommand(Receiver receiver) {
        super(receiver);
    }
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.OPEN_ALL_APPS, requestContent);
            ...
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                ...
                LOGGER.log(Level.INFO, "Open all apps command was successful");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with open all apps command: " + e.getMessage());
        }
        return router;
    }
}
