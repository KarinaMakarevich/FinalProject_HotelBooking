...
public class BlockUserCommand extends AbstractCommand {
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.BLOCK_USER, requestContent);
            Set<String> set = requestContent.getRequestAttributes().keySet();
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                router.setPath(GeneralConstant.CommandConstant.OPEN_ALL_USERS);
                ...
                LOGGER.log(Level.INFO, "Block user command was successful");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with block user command: " + e.getMessage());
        }
        return router;
    }
}
