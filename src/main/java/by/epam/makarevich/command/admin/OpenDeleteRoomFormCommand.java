...
public class OpenDeleteRoomFormCommand extends AbstractCommand {
    private static final Logger LOGGER = LogManager.getLogger(OpenDeleteRoomFormCommand.class);

    public OpenDeleteRoomFormCommand(Receiver receiver) {
        super(receiver);
    }
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.OPEN_DELETE_ROOM_FORM, requestContent);
            Set<String> set = requestContent.getRequestAttributes().keySet();
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                router.setPath(GeneralConstant.CommonPageConstant.ADMIN_PROFILE);
                ...
                LOGGER.log(Level.INFO, "Open delete room form was successful");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with opening delete room form: " + e.getMessage());
        }
        return router;
    }
}
