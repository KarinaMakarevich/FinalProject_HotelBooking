...
public class AddRoomCommand extends AbstractCommand {
    ...
    public AddRoomCommand(Receiver receiver) {
        super(receiver);
    }
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.ADD_ROOM, requestContent);
            Set<String> set = requestContent.getRequestAttributes().keySet();
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                router.setPath(GeneralConstant.CommandConstant.ADD_ROOM_FORM);
                ...
                LOGGER.log(Level.INFO, "Add room command was successful");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with add room command: " + e.getMessage());
        }
        return router;
    }
}
