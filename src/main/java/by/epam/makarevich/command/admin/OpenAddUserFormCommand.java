...
public class OpenAddUserFormCommand extends AbstractCommand {
    private static final Logger LOGGER = LogManager.getLogger(OpenAddUserFormCommand.class);

    public OpenAddUserFormCommand(Receiver receiver) {
        super(receiver);
    }
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.OPEN_ADD_USER_FORM, requestContent);
            ...
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                ...
                LOGGER.log(Level.INFO, "Open add user form was successful");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with opening add user form: " + e.getMessage());
        }
        return router;
    }
}
