...
public class OpenBalanceFormCommand extends AbstractCommand {
    private static final Logger LOGGER = LogManager.getLogger(OpenBalanceFormCommand.class);

    public OpenBalanceFormCommand(Receiver receiver) {
        super(receiver);
    }
    ...
    public Router execute(RequestContent requestContent) {
        ...
        try {
            receiver.doAction(CommandType.OPEN_BALANCE_FORM, requestContent);
            ...
            if (!set.contains(GeneralConstant.ErrorConstant.ERROR)) {
                ...
                LOGGER.log(Level.INFO, "Balance form is opened successfully");
            } else {
            ...
            router.setPath(GeneralConstant.CommonPageConstant.ERROR_PAGE);
            router.setType(RouterType.REDIRECT);
            LOGGER.log(Level.ERROR, "Error with opening balance form: " + e.getMessage());
        }
        return router;
    }
}
